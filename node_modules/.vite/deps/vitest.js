...
function format(...args) {
  ...
  let str = String(template).replace(formatRegExp, (x) => {
    ...
    switch (x) {
      ...
      case "%o":
        return inspect(args[i3++], { showHidden: true, showProxy: true });
      case "%O":
        return inspect(args[i3++]);
      case "%c": {
        i3++;
        return "";
      }
  ...
  suiteFn.each = function(cases, ...args) {
    ...
    return (name, fn2, options) => {
      ...
      cases.forEach((i3, idx) => {
        ...
        arrayOnlyCases ? suite2(formatTitle(_name, items, idx), () => fn2(...items), options) : suite2(formatTitle(_name, items, idx), () => fn2(i3), options);
      });
...
function createTest(fn2, context) {
  ...
  testFn.each = function(cases, ...args) {
    ...
    return (name, fn22, options) => {
      ...
      cases.forEach((i3, idx) => {
        const items = Array.isArray(i3) ? i3 : [i3];
        arrayOnlyCases ? test22(formatTitle(_name, items, idx), () => fn22(...items), options) : test22(formatTitle(_name, items, idx), () => fn22(i3), options);
      });
      this.setContext("each", void 0);
    };
  };
...
function eq(a2, b3, aStack, bStack, customTesters, hasKey2) {
  ...
  for (let i3 = 0; i3 < customTesters.length; i3++) {
    const customTesterResult = customTesters[i3](a2, b3);
...
function positionToOffset(source, lineNumber, columnNumber) {
  ...
  for (let i22 = 0; i22 < lineNumber - 1; i22++)
    start += lines[i22].length + nl;
...
async function saveInlineSnapshots(environment, snapshots) {
  const MagicString = (await import("./magic-string.es-WQZHUVRJ.js")).default;
...
function buildBySources(decoded, memos) {
  ...
  for (let i3 = 0; i3 < decoded.length; i3++) {
    ...
    for (let j = 0; j < line.length; j++) {
      ...
      insert(originalLine, memo.lastIndex = index2 + 1, [sourceColumn, i3, seg[COLUMN2]]);
    }
  }
  return sources;
}
function insert(array2, index2, value) {
  for (let i3 = array2.length; i3 > index2; i3--) {
